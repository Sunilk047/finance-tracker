...
class _AddTransactionSheetState extends State<AddTransactionSheet> {
  ...
  void _submit() {
    ...
    context.read<TransactionsBloc>().add(TransactionsAddRequested(
    ...
    return Container(
      ...
      child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          _TypeToggle(label: 'Check-In',  selected: _type == TransactionType.checkIn,  color: AppColors.income,  onTap: () => setState(() => _type = TransactionType.checkIn)),
          const SizedBox(width: 8),
          _TypeToggle(label: 'Check-Out', selected: _type == TransactionType.checkOut, color: AppColors.expense, onTap: () => setState(() => _type = TransactionType.checkOut)),
        ]),
        const SizedBox(height: 20),
        ...
        AppTextField(
          label: 'Title', hint: 'e.g. Salary, Grocery, Rent…',
          controller: _titleCtrl, textInputAction: TextInputAction.next,
          validator: (v) { if (v == null || v.trim().isEmpty) return 'Title is required'; return null; },
        ),
        const SizedBox(height: 14),

        AppTextField(
          label: 'Note (optional)', hint: 'Any additional details…',
