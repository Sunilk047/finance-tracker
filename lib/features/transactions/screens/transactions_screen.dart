...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  void initState() {
    super.initState();
    context.read<TransactionsBloc>().add(TransactionsLoadRequested(widget.account.id));
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => TransactionsBloc()..add(TransactionsLoadRequested(widget.account.id)),
      child: _TransactionsView(account: widget.account),
    );
  }
}
...
class _TransactionsView extends StatefulWidget {
  ...
  @override State<_TransactionsView> createState() => _TransactionsViewState();
}
...
class _TransactionsViewState extends State<_TransactionsView> {
          ...
          builder: (context, state) {
            ...
            if (state is TransactionsLoaded) {
              return RefreshIndicator(
                onRefresh: () async => context.read<TransactionsBloc>().add(TransactionsLoadRequested(widget.account.id)),
    ...
    return AppBar(
      title: _searchOpen
        ...
        decoration: InputDecoration(
          hintText: 'Search title, amount, note…', border: InputBorder.none,
          hintStyle: AppTextStyles.bodyMedium.copyWith(color: AppColors.ink300),
        ),
        onChanged: (v) => context.read<TransactionsBloc>().add(TransactionsSearchChanged(v)),
      )
  ...
  Widget _buildFilterRow(BuildContext context, TransactionFilter active) {
    return Padding(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: TransactionFilter.values.map((f) => Padding(
            padding: const EdgeInsets.only(right: 8),
            child: AppFilterChip(
              label: f.label, selected: active == f,
              onTap: () => context.read<TransactionsBloc>().add(TransactionsFilterChanged(f)),
            ),
          )).toList(),
        ),
      ),
  ...
  Widget _buildTransactionList(BuildContext context, TransactionsLoaded state) {
    final txList = state.filtered;
    if (txList.isEmpty) {
      return const SliverFillRemaining(child: EmptyState(icon: Icons.receipt_long_outlined, title: 'No Transactions', subtitle: 'Tap + to add your first\ncheck-in or check-out.'));
    }
      ...
      sliver: SliverList(delegate: SliverChildBuilderDelegate(
          ...
          return Padding(
            ...
            child: TransactionTile(
              transaction: tx,
              onDeleted: () => context.read<TransactionsBloc>().add(TransactionsDeleteRequested(tx.id)),
            ),
          );
        },
  ...
  List<dynamic> _groupByDate(List<Transaction> txList) {
    ...
    for (final tx in txList) {
      ...
      if (label != last) { result.add(label); last = label; }
      result.add(tx);
    }
    return result;
  }
  ...
  Widget _buildFabs(BuildContext context) {
    return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
      ...
      _FabAction(label: 'Check-Out', icon: Icons.remove_rounded, color: AppColors.expense, onTap: () => _showSheet(context, TransactionType.checkOut)),
    ]);
  }
  ...
  void _showSheet(BuildContext context, TransactionType type) {
    showModalBottomSheet(
      ...
      builder: (_) => BlocProvider.value(
        ...
        child: AddTransactionSheet(accountId: widget.account.id, initialType: type),
      ),
    );
  }

  Future<void> _exportPdf(BuildContext context, TransactionsLoaded state) async {
