...
class _AuthScreenState extends State<AuthScreen> with SingleTickerProviderStateMixin {
  ...
  void dispose() {
    ...
    _suEmailCtrl.dispose(); _suPassCtrl.dispose(); _suConfirmPassCtrl.dispose();
    super.dispose();
  }
  ...
  void _signup() {
    ...
    context.read<AuthBloc>().add(AuthSignUpRequested(email: _suEmailCtrl.text.trim(), password: _suPassCtrl.text));
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TabBarView(
                        controller: _tab,
                        children: [_buildLoginForm(loading), _buildSignupForm(loading)],
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildLoginForm(bool loading) {
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        AppTextField(
          label: 'Password', hint: '••••••••',
          controller: _passCtrl, obscureText: !_loginPassVisible,
          ...
          validator: (v) { if (v == null || v.isEmpty) return 'Password required'; return null; },
        ),
        const SizedBox(height: 28),
        AppButton(label: 'Login', loading: loading, onTap: _login),
        const SizedBox(height: 16),
        Center(child: TextButton(onPressed: () {}, child: Text('Forgot password?', style: AppTextStyles.labelMedium.copyWith(color: AppColors.primary)))),
      ]),
    );
  }
  ...
  Widget _buildSignupForm(bool loading) {
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        AppTextField(
          label: 'Email Address', hint: 'you@example.com',
          controller: _suEmailCtrl, keyboardType: TextInputType.emailAddress,
          textInputAction: TextInputAction.next,
          validator: (v) { if (v == null || v.isEmpty) return 'Email required'; if (!v.contains('@')) return 'Enter valid email'; return null; },
        ),
        const SizedBox(height: 16),
        AppTextField(
          label: 'Password', hint: '••••••••',
          controller: _suPassCtrl, obscureText: !_signupPassVisible,
          textInputAction: TextInputAction.next,
          suffix: IconButton(
        ...
        AppTextField(
          ...
          controller: _suConfirmPassCtrl, obscureText: true,
          textInputAction: TextInputAction.done,
          onFieldSubmitted: (_) => _signup(),
          validator: (v) { if (v != _suPassCtrl.text) return 'Passwords do not match'; return null; },
        ),
        const SizedBox(height: 28),
